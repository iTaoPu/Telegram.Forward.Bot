...
function formatTimeMessage(date) {
  ...
  const beijingDate = new Date(utcTimestamp + 8 * 3600000);

  const year = String(beijingDate.getFullYear());
...
// ==================== 时间推送核心逻辑 ====================
async function pushTimeMessage() {
  const now = new Date();
...
addEventListener('fetch', event => {
  ...
  if (url.pathname === WEBHOOK) {
    ...
    event.respondWith(handleSendTime(event));
...
// ==================== Cron 触发器处理（仅北京时间 6:00 - 22:00 推送）====================
...
async function handleScheduled(event) {
  try {
    // 获取当前北京时间小时
    const now = new Date();
    const beijingHour = (now.getUTCHours() + 8) % 24; // UTC+8

    // 只在 6:00 - 22:00（含）之间推送
    if (beijingHour >= 6 && beijingHour <= 22) {
      await pushTimeMessage();
      console.log('✅ Cron 时间推送成功（北京时间', beijingHour, '点）');
    } else {
      console.log('⏸️ 当前北京时间', beijingHour, '点不在推送时段 (6-22点)，跳过推送');
    }
  } catch (error) {
    console.error('❌ Cron 时间推送失败:', error);
  }
}
